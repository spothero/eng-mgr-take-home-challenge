...
You need Docker to run the project. Please run `docker-compose up -d` to start the container, it will create the database, the tables and adds the seed data.
An easy way to look into the content of the database is running `make docker.db-prompt`, that will provide the `psql` prompt, the command-line tool of PostgreSQL.
